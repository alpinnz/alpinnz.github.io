...
export  function Personal(): React.ReactElement {
    const personalItem: PersonalItem = {
        ...
        descriptions: [
            ...
            'I code using dart and javascript language, for design architecture i could use both MVP, MVVM, DDD, and Clean Architecture, Because of restructuring, since June \'21 i work as Mobile Engineer Lead at my current company , I always keep abreast of developing technologies.'
        ],
    };
    return (
        <>
            <section id="personal" className='mt-16 px-8 md:px-20 lg:px-24 py-4 gap-4'>
                <div className='text-center'>
                    <div className='mt-2 font-semibold text-4xl text-neutral-700'>{personalItem.title}</div>
                    {personalItem.descriptions.map((item, index) => (
                            <div key={index} className='mt-4 font-light text-neutral-800'>{item}</div>
                        )
                    )}
                </div>
            </section>
